...
SSL_CTX *evt_get_SSL_CTX(const evt_ctx_t *ctx) { return ctx->ctx; }

SSL *evt_get_ssl(const evt_tls_t *tls) { return tls->ssl; }
...
int evt_ctx_is_crtf_set(evt_ctx_t *t) { return t->cert_set; }

int evt_ctx_is_key_set(evt_ctx_t *t) { return t->key_set; }
...
static int evt__tls__op(evt_tls_t *conn, enum tls_op_type op, void *buf,
                        int sz) {
  int r = 0;
  int bytes = 0;
  char tbuf[16 * 1024] = {0};
